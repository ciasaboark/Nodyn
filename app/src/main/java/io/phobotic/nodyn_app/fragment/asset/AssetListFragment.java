...
import io.phobotic.nodyn_app.list.adapter.AssetModelSection;
...
public class AssetListFragment extends Fragment {
    ...
    private void showProperView() {
        ...
        //load a list of assets in the background
        new Handler().post(new Runnable() {
            @Override
            public void run() {
                final List<SimplifiedAsset> assetList = new ArrayList<>();

                try {
                    Database db = Database.getInstance(getContext());
                    List<Asset> list = db.getAssets();
    ...
    private void showDefaultAssetList() {
        AssetListFilterer filterer = new AssetListFilterer(getContext());
        filterer.filterList(assets, null, true, new AssetListFilterer.AssetListFilterListener() {
            @Override
    ...
    private void init() {
        ...
        filterCountBox = rootView.findViewById(R.id.filter_count_box);
        filterCount = rootView.findViewById(R.id.filter_count);
        ...
        searchBar = rootView.findViewById(R.id.inner_toolbar);
    ...
    private void filterListAndShow(String filter) {
        AssetListFilterer filterer = new AssetListFilterer(getContext());
        filterer.filterList(assets, filter, false, new AssetListFilterer.AssetListFilterListener() {
            @Override
            public void onAssetListBeginFilter() {
                Activity a = getActivity();
                if (a != null) {
    ...
    private class ListMapperTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            for (SectionKey key : assetSectionMap.keySet()) {
                ...
                sectionedAdapter.addSection(new AssetModelSection(getContext(), key.getManufacturer(),
