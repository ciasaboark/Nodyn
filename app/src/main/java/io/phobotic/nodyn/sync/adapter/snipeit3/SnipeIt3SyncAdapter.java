...
import io.phobotic.nodyn.sync.Link;
...
import io.phobotic.nodyn.sync.adapter.snipeit3.response.CategoryResponse;
import io.phobotic.nodyn.sync.adapter.snipeit3.response.GroupResponse;
...
public class SnipeIt3SyncAdapter implements SyncAdapter {
    ...
    /**
     * Converts any fields marked with {@link HtmlEncoded} or {@link io.phobotic.nodyn.sync.Link}
    ...
    private void sendProgressBroadcast(Context context, int progress) {
        Intent i = getBroadcastIntent();
        LocalBroadcastManager broadcastManager = LocalBroadcastManager.getInstance(context);
        i.putExtra(SyncService.BROADCAST_SYNC_PROGRESS, progress);
        broadcastManager.sendBroadcast(i);
    }
